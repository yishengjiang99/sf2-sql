...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < h2->size; i++) fgetc(fd);
  ...
  fread(h2, sizeof(header2_t), 1, fd);
  printf("\n%.4s %u\n", h2->name, h2->size);
  int npresets, npbags, npgens, npmods, nshdrs, ninst, nimods, nigens, ishdrs,
      nibags;
    ...
    for (int j = phdrs[i].pbagNdx; j < lastbag; j++) {
      ...
      for (int k = pgenId; k < lastPgenId; k++) {
        ...
        if (pge->operator== 44 || pge->operator== 43) {
          printf(
              "\n%d, pbag, %d, %s, %d,%s,val, %hu-%hu", phdrs[i].pid, j,
              phdrs[i].name, pge->operator,
              generator[pge->operator], pge->val.ranges.lo, pge->val.ranges.hi);
        } else {
          printf("\n%d, pbag, %d, %s, %d, %s, val, %d", phdrs[i].pid, j,
                 phdrs[i].name, pge->operator,
                 generator[pge->operator], pge->val.shAmount);
        }
        if (pge->operator== 41) {
          instID = pge->val.uAmount;
        }
      }
      if (instID != -1) {
        ...
        for (int ibg = ibgId; ibg < lastibg; ibg++) {
          ...
          pgen_t *lastig = ibg < nibags - 1 ? igens + (ibgg + 1)->igen_id
                                            : igens + nigens - 1;
          for (pgen_t *ig = igens + ibgg->igen_id;
               ig->operator!= 60 && ig != lastig; ig++) {
            if (ig->operator== 44 || ig->operator== 43) {
              printf("\n%d, inst, %d, %d, %s, ", phdrs[i].pid, ibg, instID,
                     ihead->name);
              printf("%d, %s,val, %hu-%hu", ig->operator,
                     generator[ig->operator], ig->val.ranges.lo,
                               ig->val.ranges.hi);
            } else {
              printf("\n%d, inst, %d, %d, %s, ", phdrs[i].pid, ibg, instID,
                     ihead->name);
              printf("%hu, %s, val, %d", ig->operator,
                     generator[ig->operator], ig->val.shAmount);
            }
            if (ig->operator== 53) {
              lastSampId = ig->val.uAmount;

              shdrcast *sample = (shdrcast *)(shdrs + lastSampId * 46);

              printf("\n%d, samp, %d, %d, %.20s, %u,%u,%u,%u,%u", phdrs[i].pid,
                     lastSampId, instID, sample->name, sample->start,
                     sample->end, sample->startloop, sample->endloop,
                     sample->sampleRate);
            }
          }
        }
